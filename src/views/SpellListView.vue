<template>
  ...
  <div class="container spell-view pt-3">
    ...
    <div class="d-flex spell-buttons align-items-center fs-3 d-print-none">
      <button
        type="button"
        class="btn btn-link text-muted me-3 spell-button p-0"
      ...
      <ul class="dropdown-menu" aria-labelledby="spellSettings">
        <li>
          <div class="dropdown-item" @click="toggleFilter('prepared', $event)">
            <i :class="filterPrepared ? 'bi bi-check text-primary' : 'bi bi-x'"> </i>
            Filter: Prepared spells ({{ $md.ply.magic.getPreparedSpells().length }})
          </div>
        </li>
        <li>
          <div class="dropdown-item" @click="toggleFilter('ritual', $event)">
            <i :class="filterRitual ? 'bi bi-check text-primary' : 'bi bi-x'"> </i>
            Filter: Ritual spells
          </div>
        </li>
      </ul>
    </div>
      ...
      <div v-for="group in getSpellsByLevel($md.ply.magic.spells)" :key="group[0]">
        ...
        <ul class="list-group list-group-flush mb-3">
          ...
          <div class="my-1 py-2 spell-item" v-for="spell in group[1]" :key="spell.id">
            <router-link
              class="text-decoration-none d-block p-3 py-0 spell-link"
              ...
              <div class="row align-items-center">
                <div class="col text-primary">
                  ...
                  <span
                    ...
                    title="This spell is prepared"
                    v-if="spell.prepared"
                    >P</span
                  >
                  <span
                    class="text-secondary ps-1"
                    title="Can be ritual casted"
                    v-if="spell.ritual == 'yes'"
                    >R</span
                  >
                </div>
                ...
                <div class="col-1 d-print-none">
                  <div class="dropdown text-end">
                    <button
                      class="btn text-muted px-0 py-0 px-sm-3"
                      type="button"
                      :id="`dropdownMenuButton-${spell.id}`"
                      data-bs-toggle="dropdown"
                      aria-expanded="false"
                    >
                    ...
                    <ul
                      class="dropdown-menu"
                      :aria-labelledby="`dropdownMenuButton-${spell.id}`"
                    >
                      <li>
                        <div
                          class="dropdown-item"
  ...
  <div
    ...
    id="createSpellModal"
    tabindex="-1"
    aria-labelledby="createSpellModalLabel"
    aria-hidden="true"
  >
      ...
      <div class="modal-content">
        <form @submit.prevent="submitSpell">
          <div class="modal-header">
            <h5 class="modal-title" id="createSpellModalLabel">
              {{ isEditing ? "Edit Spell" : "Add a new spell" }}
            </h5>
            <button
              type="button"
              class="btn-close"
...
export default {
  ...
  methods: {
    ...
    openEditModal(spell, event) {
      if (event) event.preventDefault();
      this.isEditing = true;
      this.oldSpellName = spell.name;
      // Clone the spell data so we don't mutate state directly before submitting
      this.newSpell = { ...spell };
    },
    ...
    submitSpell() {
      ...
      // Attempt to add or edit the spell.
      try {
        // If editing, remove the old version of the spell first
        // (This is necessary because the spell map is keyed by spell.name)
        if (this.isEditing && this.oldSpellName) {
          this.$md.ply.magic.spells.delete(this.oldSpellName);
        }
        ...
        // Optionally navigate to the spell's detail page, or stay on the list if editing
        if (!this.isEditing) {
        ...
        alert(
          "There was an error saving the spell. Please check the input and try again."
        );
      }
    },
  },
